...
import '../leave/leave'
import { generateRandomDateAfterToday } from "../generateRandomDate";
...
describe('Leave Table Components',()=>{
    ...
    it.only('Create the new leave via employee list',()=>{
        let randomDate=generateRandomDateAfterToday()
        cy.get('.h-full').should('be.visible').click()
        cy.get("#headlessui-dialog-panel-3 > nav > div > ol:nth-child(2) > li > div > div > button > span.ml-4.font-medium").click()
...
it('Negative Assertion for Bulk Upload',()=>{
  let p='3.jpg'
  cy.get('.h-full').should('be.visible').click();
  cy.get("#headumlessui-dialog-panel-3 > nav > div > ol:nth-child(2) > li > div > div > button > span.ml-4.font-medium").click();
